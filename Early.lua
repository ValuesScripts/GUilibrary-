...
function handle:AddToggle(Config)
    ...
    -- Gear (left side, hidden initially)
    ...
    GearButton.Position = UDim2.new(0, 2, 0.5, 0)
    ...
    GearButton.Visible = false   -- hidden until extras are added
    ...
    -- Create ToggleLib BEFORE we use it in callbacks
    local ToggleLib = {
        Root = Toggle,
        CheckMark = CheckMark,
        UIStroke = UIStroke,
    ...
    ToggleLib.SetVisible = LPH_NO_VIRTUALIZE(function(value)
        if value then
            ...
            if ToggleLib.ExtraHandler then
                ToggleLib.ExtraHandler.SetVisible(false)
            end
        end
    end)
    ...
    -- Connect to the main signal
    ToggleLib.Signal = Signal:Connect(ToggleLib.SetVisible)

    -- Extra window (created lazily)
    local hasExtras = false

    -- Click on gear
    GearHit.MouseButton1Click:Connect(function()
        if not hasExtras then return end
        if not ToggleLib.ExtraHandler then
            ToggleLib.ExtraHandler = NeverLose:CreateExtraWindow(Toggle, Toggle.ZIndex)
            -- Also create a signal to close when clicking outside
            local closeSignal = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if ToggleLib.ExtraHandler and ToggleLib.ExtraHandler.Root.Visible then
                        local root = ToggleLib.ExtraHandler.Root
            ...
            -- Store the signal so it can be cleaned up if needed
            ToggleLib._closeSignal = closeSignal
        end
        ToggleLib.ExtraHandler.SetVisible(not ToggleLib.ExtraHandler.Root.Visible)
    end)

    -- Checkbox click area (right side, avoids the gear)
    ...
    CheckboxButton.Position = UDim2.new(0, 22, 0, 0)   -- same as CheckMark
    CheckboxButton.ZIndex = ZINdex + 17
    CheckboxButton.Text = ""
    CheckboxButton.TextTransparency = 1
