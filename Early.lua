...
function NeverLose:CreateExtraWindow(AnchorFrame, ZIndex)
    local ExtraWindow = Instance.new("Frame")
    local UICorner = Instance.new("UICorner")
    local UIStroke = Instance.new("UIStroke")
    local UIListLayout = Instance.new("UIListLayout")
    local Shadow = NeverLose:CreateShadow(ExtraWindow)

    ExtraWindow.Name = NeverLose.RandomString()
    ExtraWindow.Parent = NeverLose.ScreenGui
    ExtraWindow.AnchorPoint = Vector2.new(0.5, 0)
    ExtraWindow.BackgroundColor3 = Color3.fromRGB(20, 22, 27)
    ExtraWindow.BackgroundTransparency = 1
    ExtraWindow.BorderSizePixel = 0
    ExtraWindow.ClipsDescendants = true
    ExtraWindow.Position = UDim2.new(0, -9999, 0, 0)
    ExtraWindow.Size = UDim2.new(0, 220, 0, 50)
    ExtraWindow.ZIndex = ZIndex + 50
    ExtraWindow.Visible = false

    UICorner.CornerRadius = UDim.new(0, 10)
    UICorner.Parent = ExtraWindow

    UIStroke.Transparency = 0.650
    UIStroke.Color = Color3.fromRGB(45, 48, 58)
    UIStroke.Parent = ExtraWindow

    UIListLayout.Parent = ExtraWindow
    UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
    ...
    UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
        local h = UIListLayout.AbsoluteContentSize.Y + 10
        ExtraWindow.Size = UDim2.new(0, 220, 0, math.max(h, 20))
    end)

    -- Show/hide logic
    local function SetVisible(visible)
        if visible then
            local pos = AnchorFrame.AbsolutePosition
            ExtraWindow.Position = UDim2.new(0, pos.X + AnchorFrame.AbsoluteSize.X + 10, 0, pos.Y)
            ExtraWindow.Visible = true
            ExtraWindow.Parent = NeverLose.ScreenGui
            Shadow:Render(true)
            TweenService:Create(ExtraWindow, SlowyTween, { BackgroundTransparency = 0.035 }):Play()
            TweenService:Create(UIStroke, SlowyTween, { Transparency = 0.650 }):Play()
        else
            Shadow:Render(false)
            TweenService:Create(ExtraWindow, SlowyTween, { BackgroundTransparency = 1 }):Play()
            TweenService:Create(UIStroke, SlowyTween, { Transparency = 1 }):Play()
            task.wait(0.2)
            ExtraWindow.Visible = false
            ExtraWindow.Parent = nil
        end
    end
    ...
    -- Create a handler for the extra window
    ExtraLib.__handler = NeverLose:RegisiterItem(ExtraWindow, NeverLose:CreateSignal(true))

    -- Override the handler's SetRender to actually show/hide the window
    ExtraLib.__handler.SetRender = function(value)
        SetVisible(value)
    end

    -- Expose the handler methods
    function ExtraLib:AddToggle(...) return ExtraLib.__handler:AddToggle(...) end
    function ExtraLib:AddSlider(...) return ExtraLib.__handler:AddSlider(...) end
    function ExtraLib:AddDropdown(...) return ExtraLib.__handler:AddDropdown(...) end
    function ExtraLib:AddColorPicker(...) return ExtraLib.__handler:AddColorPicker(...) end
    function ExtraLib:AddKeybind(...) return ExtraLib.__handler:AddKeybind(...) end
    function ExtraLib:AddTextInput(...) return ExtraLib.__handler:AddTextInput(...) end
    function ExtraLib:AddButton(...) return ExtraLib.__handler:AddButton(...) end
    function ExtraLib:AddLabel(...) return ExtraLib.__handler:AddLabel(...) end
...
function handle:AddToggle(Config)
    ...
    -- +++++ GEAR BUTTON (left side) +++++
    local GearButton = Instance.new("TextLabel")
    local GearHit = Instance.new("TextButton")
    -- +++++++++++++++++++++++++++++++++++

    Toggle.Name = NeverLose.RandomString()
    Toggle.Parent = Handler
    Toggle.BackgroundColor3 = Color3.fromRGB(26, 28, 36)
    Toggle.BorderColor3 = Color3.fromRGB(0, 0, 0)
    Toggle.BorderSizePixel = 0
    Toggle.ClipsDescendants = true
    -- +++++ Increased width to fit gear + checkbox + spacing +++++
    ...
    -- +++++ GEAR (left side, hidden by default) +++++
    GearButton.Name = NeverLose.RandomString()
    GearButton.Parent = Toggle
    GearButton.AnchorPoint = Vector2.new(0, 0.5)
    GearButton.BackgroundTransparency = 1
    GearButton.Position = UDim2.new(0, 2, 0.5, 0)   -- left edge
    GearButton.Size = UDim2.new(0, 16, 0, 16)
    GearButton.ZIndex = ZINdex + 15
    GearButton.FontFace = NeverLose.BuiltInBold
    GearButton.Text = "gear"
    GearButton.TextColor3 = Color3.fromRGB(223, 223, 223)
    GearButton.TextSize = 14
    GearButton.TextTransparency = 0.6
    GearButton.TextWrapped = true
    GearButton.Visible = false  -- hidden until extras added

    -- Invisible hitbox for the gear
    GearHit.Name = NeverLose.RandomString()
    GearHit.Parent = Toggle
    GearHit.AnchorPoint = Vector2.new(0, 0.5)
    GearHit.Position = UDim2.new(0, 0, 0.5, 0)
    GearHit.Size = UDim2.new(0, 20, 0, 20)
    GearHit.BackgroundTransparency = 1
    GearHit.ZIndex = ZINdex + 16
    GearHit.Text = ""
    GearHit.Visible = false

    -- Checkbox (now on the right of the gear)
    CheckMark.Name = NeverLose.RandomString()
    CheckMark.Parent = Toggle
    CheckMark.AnchorPoint = Vector2.new(0, 0.5)
    CheckMark.BackgroundTransparency = 1
    -- Position to the right of the gear (with spacing)
    CheckMark.Position = UDim2.new(0, 22, 0.5, 0)
    ...
    -- +++++ Extra window handler (created lazily) +++++
    local ExtraHandler = nil
    local ExtraSignal = nil
    local hasExtras = false  -- tracks if any extra modules were added

    -- Click on gear to toggle extra window
    GearHit.MouseButton1Click:Connect(function()
        if not hasExtras then return end  -- should not happen if hidden, but safety
        if not ExtraHandler then
            ExtraHandler = NeverLose:CreateExtraWindow(Toggle, Toggle.ZIndex)
            ExtraSignal = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if ExtraHandler and ExtraHandler.Root.Visible then
                        local root = ExtraHandler.Root
                        if not NeverLose:IsMouseOverFrame(root) and not NeverLose:IsMouseOverFrame(Toggle) then
                            ExtraHandler.SetVisible(false)
                        end
                    end
                end
            end)
        end
        ExtraHandler.SetVisible(not ExtraHandler.Root.Visible)
    end)

    -- Click on checkbox to toggle state (only the checkbox area)
    local CheckboxHit = NeverLose:CreateInput(Toggle, LPH_NO_VIRTUALIZE(function()
        Config.Default = not Config.Default
        ToggleLib.SetUI(Config.Default)
        Config.Callback(Config.Default)
    end))
    -- Adjust the input area to cover only the checkbox (not the gear)
    -- We can do this by making the input frame cover the right portion.
    -- In CreateInput, it covers the whole frame, so we'll manually create a button for the checkbox.
    -- Better: remove the previous CreateInput and add a new button for checkbox only.
    -- Let's delete the previous input and create a custom one.
    -- We'll remove the CreateInput call and instead create a new ImageButton for checkbox.

    -- So replace the earlier NeverLose:CreateInput with this:
    local CheckboxButton = Instance.new("ImageButton")
    CheckboxButton.Name = NeverLose.RandomString()
    CheckboxButton.Parent = Toggle
    CheckboxButton.BackgroundTransparency = 1
    CheckboxButton.Size = UDim2.new(0, 18, 0, 18)   -- matches checkbox area
    CheckboxButton.Position = UDim2.new(0, 22, 0, 0) -- same as CheckMark position
    CheckboxButton.ZIndex = ZINdex + 17
    CheckboxButton.ImageTransparency = 1
    CheckboxButton.Text = ""

    CheckboxButton.MouseButton1Click:Connect(LPH_NO_VIRTUALIZE(function()
        Config.Default = not Config.Default
        ToggleLib.SetUI(Config.Default)
        Config.Callback(Config.Default)
    end))
    ...
    ToggleLib.SetUI = LPH_NO_VIRTUALIZE(function(value)
        if value then
            TweenService:Create(Toggle, SlowyTween, { BackgroundColor3 = NeverLose.AccentColor, BackgroundTransparency = 0 }):Play()
            TweenService:Create(UIStroke, SlowyTween, { Transparency = 0 }):Play()
            TweenService:Create(CheckMark, SlowyTween, { TextTransparency = 0 }):Play()
        else
            TweenService:Create(Toggle, SlowyTween, { BackgroundColor3 = Color3.fromRGB(26, 28, 36), BackgroundTransparency = 0 }):Play()
            TweenService:Create(UIStroke, SlowyTween, { Transparency = 0.650 }):Play()
            TweenService:Create(CheckMark, SlowyTween, { TextTransparency = 1 }):Play()
        end
    end)
    ...
    ToggleLib.SetVisible = LPH_NO_VIRTUALIZE(function(value)
        if value then
            ...
            TweenService:Create(Toggle, SlowyTween, { BackgroundTransparency = 1 }):Play()
            -- close extra window if open
            if ExtraHandler then ExtraHandler.SetVisible(false) end
        end
        -- Also hide gear when toggle is hidden? Actually we want to keep gear hidden if no extras
        -- but we'll manage visibility separately.
    end)

    ToggleLib.SetUI(Config.Default)
    ToggleLib.SetVisible(Signal:GetValue())

    -- Connect to the main signal for visibility
    ToggleLib.Signal = Signal:Connect(ToggleLib.SetVisible)

    -- +++++ Extra() method +++++
    function ToggleLib:Extra()
        -- Create the extra window if not done yet
        if not ExtraHandler then
            ExtraHandler = NeverLose:CreateExtraWindow(Toggle, Toggle.ZIndex)
            ExtraSignal = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Mark that extras exist and show the gear
        if not hasExtras then
            hasExtras = true
            GearButton.Visible = true
            GearHit.Visible = true
        end
        return ExtraHandler
    end
    -- +++++++++++++++++++++++++++++++++++++++
